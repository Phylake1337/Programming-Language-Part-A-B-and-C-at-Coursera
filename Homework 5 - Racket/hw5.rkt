...
(define (eval-under-env e env)
  (cond [(int? e) e]
        ...
        [(mlet? e) (let*([env (cons (cons (mlet-var e) (eval-under-env (mlet-e e) env)) env)])
        ...
        [(fst? e) (let([myPair (eval-under-env (fst-e e) env)])
                       (if (apair? myPair)
                           (apair-e1 myPair)
                           (error"The arg isn't a pair")))]
        
        [(snd? e) (let([myPair (eval-under-env (snd-e e) env)])
                       (if (apair? myPair)
                           (apair-e2 myPair)
                           (error"The arg isn't a pair")))]
