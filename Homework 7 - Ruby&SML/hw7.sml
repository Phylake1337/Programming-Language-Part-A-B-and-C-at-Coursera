...
fun pp_helper (x1,y1,x2,y2) = if x2 < x1 
...
fun preprocess_prog e =
	...
	NoPoints  => e
      ...
      | Let (str,exp1,exp2) => Let(str, preprocess_prog exp1, preprocess_prog exp2)
      | Var (str) => e
      | Shift (r1,r2,exp) => Shift (r1,r2, preprocess_prog exp)
      | LineSegment(x1,y1,x2,y2) => if real_close_point (x1,y1) (x2,y2)
				    then Point(x1, y1)
				    else pp_helper(x1,y1,x2,y2)

fun shift_handle (dx,dy,e) =
    case e of
	NoPoints => e
...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      | Shift (dx,dy,exp) => shift_handle(dx, dy, exp)
